...
const char * const BITCOIN_CONF_FILENAME = "nirigecoin.conf";
...
#else
    const char* pszModule = "nirigecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nirigecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Nirigecoin";
#else
    // Unix
    return pathRet / ".nirigecoin";
#endif
#endif
}
